...
<%@ Page Language="vb" AutoEventWireup="false" Codebehind="wf_cambiturnoNecessario.aspx.vb" Inherits="ATCBussm1.wf_cambiturnoNecessario"%>
...
<HTML>
	...
	<body>
		<form id="Form1" method="post" runat="server">
			...
			<table id="Table01" class="Back" cellSpacing="0" cellPadding="0" width="100%" align="center">
				...
				<tr>
					<td style="PADDING-BOTTOM: 0px; PADDING-LEFT: 5px; PADDING-RIGHT: 5px; PADDING-TOP: 0px"
						vAlign="top">
						<h6>Scambi turno Necessario <span class="label">(Sono visibili solamente gli scambi 
								turno "Necessari" ancora da effettuare)</span></h6>
						<hr SIZE="1" noShade>
						<asp:validationsummary id="vsErrori" runat="server" CssClass="error" Width="100%" HeaderText="Attenzione!"></asp:validationsummary><asp:label id=lblCambiTurno runat="server" CssClass="label" Visible='<%# DataBinder.Eval(DsCambiTurnoNecessario1, "Tables[RichiesteCambiTurno]").Rows.Count <= 0 %>'>Non ci sono scambi turno.</asp:label><asp:button id="btnNuovaRichiesta" runat="server" CssClass="bottone" Text="Nuovo scambio turno Necessario"></asp:button><asp:panel id="pnlData" runat="server" Visible="False">
							<HR SIZE="1" noShade>
							<H6>Quando?</H6>
							<HR SIZE="1" noShade>
							<asp:DropDownList style="Z-INDEX: 0" id="ddlData" runat="server"></asp:DropDownList>
							<asp:CustomValidator id="cvData" runat="server" ErrorMessage="Le richieste di scambio consensuale di turno sono consentite fino alle ore 18:00 del giorno precedente a quello interessato dalla richiesta."
							...
							<asp:CustomValidator id="cvEccezioni" runat="server" ErrorMessage="Superato numero scambi giornalieri nel mese"
								Display="None"></asp:CustomValidator>
							<asp:CustomValidator id="cvTurnoOkGiornaliero" runat="server" ErrorMessage="Non hai un turno associato in questa data."
								Display="None"></asp:CustomValidator>
							<asp:CustomValidator id="cvSoloUnCambio" runat="server" ErrorMessage="E' possibile richiedere solamente uno scambio turno Necessario al mese."
								Display="None"></asp:CustomValidator>
							<asp:CustomValidator id="cvGiornalieroAbilitato" runat="server" ErrorMessage="Giornaliero non rilasciato per questa data."
								Display="None"></asp:CustomValidator>
							<asp:CustomValidator id="cvbloccoCambio" runat="server" ErrorMessage="Funzionalità inibita." Display="None"></asp:CustomValidator>
							<asp:CustomValidator id="cvCheckPatente" runat="server" ErrorMessage="Turno richiede possesso patente"
								Display="None"></asp:CustomValidator>
							<HR SIZE="1" noShade>
						</asp:panel><asp:datagrid id=dgCambiTurnoGiornalieri runat="server" CssClass="grid" Width="100%" Visible='<%# DataBinder.Eval(DsCambiTurnoNecessario1, "Tables[RichiesteCambiTurno]").Rows.Count > 0 %>' GridLines="None" AutoGenerateColumns="False" DataKeyField="IdRichiesta" DataMember="RichiesteCambiTurno" DataSource="<%# DsCambiTurnoNecessario1 %>" UseAccessibleHeader="True">
							<Columns>
								<asp:TemplateColumn>
								...
								<asp:TemplateColumn HeaderText="Con">
									...
									<EditItemTemplate>
										<asp:DropDownList id=ddlDestinatarioCambioTurno runat="server" DataSource="<%# DsCambiTurnoNecessario1 %>" DataMember="Accertatori" DataValueField="Matricola" DataTextField="Accertatore" SelectedValue='<%# DataBinder.Eval(Container, "DataItem.MatricolaAccertatoreDestinatario") %>'>
										</asp:DropDownList>
									</EditItemTemplate>
								</asp:TemplateColumn>
			...
			<table id="Table02" class="Back" cellSpacing="0" cellPadding="0" width="100%" align="center">
				...
				<tr>
					<td style="PADDING-BOTTOM: 0px; PADDING-LEFT: 5px; PADDING-RIGHT: 5px; PADDING-TOP: 0px"
						...
						<hr SIZE="1" noShade>
						<asp:label id=lblCambiTurnoDaAccettare runat="server" CssClass="label" Visible='<%# DataBinder.Eval(DsCambiTurnoDaAccettare1, "Tables[RichiesteCambiTurnoNecessario]").Rows.Count <= 0 %>'>Non ci sono scambi turno da accettare.</asp:label><asp:datagrid style="Z-INDEX: 0" id=dgCambiTurnoDaAccettare runat="server" CssClass="grid" Width="100%" Visible='<%# DataBinder.Eval(DsCambiTurnoDaAccettare1, "Tables[RichiesteCambiTurnoNecessario]").Rows.Count > 0 %>' GridLines="None" AutoGenerateColumns="False" DataKeyField="IdRichiesta" DataMember="RichiesteCambiTurnoNecessario" DataSource="<%# DsCambiTurnoDaAccettare1 %>" UseAccessibleHeader="True">
							<Columns>
								...
								<asp:TemplateColumn HeaderText="Nel giorno">
									<ItemTemplate>
										<asp:Label id=lbldata Text='<%# DataBinder.Eval(Container, "DataItem.data", "{0:d}") %>' Runat="server">
										</asp:Label>
										<asp:CustomValidator id="cvdatadaaccettare" runat="server" OnServerValidate="cvdatadaaccettare_OnServerValidate"
											Display="None" ErrorMessage="Non è più consentito accettare questa richiesta di scambio turno necessario."></asp:CustomValidator>
										<asp:CustomValidator id="cvblocco" runat="server" OnServerValidate="cvblocco_OnServerValidate" Display="None"
											ErrorMessage="Funzionalità inibita."></asp:CustomValidator>
									</ItemTemplate>
								</asp:TemplateColumn>
								...
								<asp:TemplateColumn>
									<HeaderStyle Width="1%"></HeaderStyle>
									<ItemTemplate>
										<asp:Button id="Button1" runat="server" CssClass="bottone" Text="Accetta" CausesValidation="false"
			...
			<asp:customvalidator id="cvTipoUtente" runat="server" ErrorMessage="Solamente gli accertatori possono richiedere congedi e scambi turno."
				Display="None"></asp:customvalidator><asp:customvalidator id="cvOnModifica" runat="server" ErrorMessage="Non è possibile modificare questa richiesta perché non sei tu il richiedente oppure perché è stata accettata o rifiiutata."
